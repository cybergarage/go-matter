...
type Header struct {
	...
	// SourceNodeID is optional, present when the header indicates a source node ID is included.
	SourceNodeID uint64
	// DestNodeID is optional, present when the header indicates a destination node ID is included.
	DestNodeID uint64
}
...
// Encode serializes the header to bytes (little-endian).
...
// String returns a human-readable representation with a hex dump.
