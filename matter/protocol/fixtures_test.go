	...
	tests := []struct {
	...
	}{
		{
			...
			validateFunc: func(t *testing.T, msg Message) {
				...
				if msg.Opcode() != 0x20 {
					t.Errorf("Expected opcode 0x20, got 0x%02X", msg.Opcode())
				}
				if msg.ProtocolID() != 0x0000 {
					t.Errorf("Expected protocolID 0x0000 (SecureChannel), got 0x%04X", msg.ProtocolID())
				}
			},
		},
		{
