...
// spake2pM and spake2pN are the fixed SPAKE2+ generator points for P-256 (uncompressed SEC1, 65 bytes).
...
// CryptoPAKERandomScalar generates a uniformly-random scalar suitable for SPAKE2+ (32 bytes, P-256 group size).
// 3.10. Password-Authenticated Key Exchange (PAKE).
func CryptoPAKERandomScalar() ([]byte, error) {
	scalar := make([]byte, CryptoGroupSizeBytes)
...
// CryptoPA returns the SPAKE2+ initiator public value pA = x·P + w0·M.
// x is a random scalar generated by CryptoPAKERandomScalar, w0 is derived by CryptoPAKEValuesInitiator.
// 3.10.1. Computation of pA.
func CryptoPA(x, w0 []byte) ([]byte, error) {
	if len(x) != CryptoGroupSizeBytes {
		return nil, newErrInvalidLen("x", CryptoGroupSizeBytes, len(x))
	}
	if len(w0) != CryptoGroupSizeBytes {
		return nil, newErrInvalidLen("w0", CryptoGroupSizeBytes, len(w0))
	}
	curve := ellipticCurve
	// Compute x*P (x times base point)
	xPx, xPy := curve.ScalarBaseMult(x)
	// Unmarshal M generator point
	Mx, My := elliptic.Unmarshal(curve, spake2pM)
	if Mx == nil {
		return nil, errors.New("failed to unmarshal SPAKE2+ M generator")
	}
	// Compute w0*M
	w0Mx, w0My := curve.ScalarMult(Mx, My, w0)
	// pA = x*P + w0*M
	pAx, pAy := curve.Add(xPx, xPy, w0Mx, w0My)
	if pAx == nil {
		return nil, errors.New("failed to compute pA")
	}
	out := elliptic.Marshal(curve, pAx, pAy)
	if len(out) != CryptoPublicKeySizeBytes {
		return nil, newErrInvalidLen("pA", CryptoPublicKeySizeBytes, len(out))
	}
	return out, nil
}

// CryptoPB returns the SPAKE2+ responder public value pB = y·P + w0·N.
// y is the responder's ephemeral random scalar (generated by
// CryptoPAKERandomScalar, mirroring CryptoPA's x), w0 is derived by
// CryptoPAKEValuesResponder. L (the other value CryptoPAKEValuesResponder
// returns) is NOT an input here — it is only used later, in
// CryptoPAKESharedPointsResponder's V = y·L.
// 3.10.2. Computation of pB.
//
// This used to just validate-and-return L (w1·P, a value fixed by the
// passcode alone, identical on every session) as if it were pB — never
// generating or using an ephemeral y at all. That produced a correctly
// shaped, deterministic 65-byte point, so every existing self-consistency
// test passed, but connectedhomeip's own ComputeRoundOne (VERIFIER role,
// src/crypto/CHIPCryptoPAL.cpp) computes pB as y·P + w0·N with a fresh
// per-session y — a real (or correct) initiator would derive different
// Z/V/session keys than this responder ever could, and pB itself would
// leak the passcode-derived w1·P unchanged across every session. The only
// caller of this function in the whole repo (pake2_message.go's
// NewPake2Message, itself never invoked by any production code — only by
// exploratory test code) was already broken for an unrelated reason
// (computeCB substitutes stand-in values for Z/V), so fixing the signature
// here does not change any working behavior.
func CryptoPB(y, w0 []byte) ([]byte, error) {
	if len(y) != CryptoGroupSizeBytes {
		return nil, newErrInvalidLen("y", CryptoGroupSizeBytes, len(y))
	}
	if len(w0) != CryptoGroupSizeBytes {
		return nil, newErrInvalidLen("w0", CryptoGroupSizeBytes, len(w0))
	}
	curve := ellipticCurve
	// Compute y*P (y times base point)
	yPx, yPy := curve.ScalarBaseMult(y)
	// Unmarshal N generator point
	Nx, Ny := elliptic.Unmarshal(curve, spake2pN)
	if Nx == nil {
		return nil, errors.New("failed to unmarshal SPAKE2+ N generator")
	}
	// Compute w0*N
	w0Nx, w0Ny := curve.ScalarMult(Nx, Ny, w0)
	// pB = y*P + w0*N
	pBx, pBy := curve.Add(yPx, yPy, w0Nx, w0Ny)
	if pBx == nil {
		return nil, errors.New("failed to compute pB")
	}
	out := elliptic.Marshal(curve, pBx, pBy)
	if len(out) != CryptoPublicKeySizeBytes {
		return nil, newErrInvalidLen("pB", CryptoPublicKeySizeBytes, len(out))
	}
	return out, nil
}

// CryptoPAKESharedPoints computes the SPAKE2+ shared elliptic-curve points Z and V for the initiator.
...
func CryptoPAKESharedPoints(x, w0, w1, pB []byte) (Z []byte, V []byte, err error) {
	...
	if len(w0) != CryptoGroupSizeBytes {
		return nil, nil, newErrInvalidLen("w0", CryptoGroupSizeBytes, len(w0))
	}
	if len(w1) != CryptoGroupSizeBytes {
		return nil, nil, newErrInvalidLen("w1", CryptoGroupSizeBytes, len(w1))
	}
	curve := ellipticCurve
	// Unmarshal pB
	pBx, pBy := elliptic.Unmarshal(curve, pB)
	if pBx == nil {
		return nil, nil, fmt.Errorf("failed to unmarshal pB")
	}
	// Unmarshal N generator
	Nx, Ny := elliptic.Unmarshal(curve, spake2pN)
	if Nx == nil {
		return nil, nil, errors.New("failed to unmarshal SPAKE2+ N generator")
	}
	// Compute w0*N
	w0Nx, w0Ny := curve.ScalarMult(Nx, Ny, w0)
	// Negate w0*N: (x, y) -> (x, p-y)
	p := curve.Params().P
	negW0Ny := new(big.Int).Sub(p, w0Ny)
	negW0Ny.Mod(negW0Ny, p)
	// tmp = pB - w0*N = pB + (-w0*N)
	tmpX, tmpY := curve.Add(pBx, pBy, w0Nx, negW0Ny)
	// Z = x * tmp
...
// CryptoPAKESharedPointsResponder computes the SPAKE2+ shared elliptic-curve
// points Z and V for the responder (Verifier). y is the responder's
// ephemeral random scalar (the same one used to compute pB via CryptoPB),
// w0 and l are derived by CryptoPAKEValuesResponder, and pA is the
// initiator's public value received in Pake1.
//
// This mirrors the initiator's CryptoPAKESharedPoints, but with the roles
// of M/N swapped (tmp = pA - w0·M here, vs. pB - w0·N there) and V computed
// as y·L directly rather than w1·tmp — the responder holds only w0 and L
...
func CryptoPAKESharedPointsResponder(y, w0, l, pA []byte) (Z []byte, V []byte, err error) {
	...
	if pAx == nil {
		return nil, nil, fmt.Errorf("failed to unmarshal pA")
	}
	// Unmarshal L (a curve point, w1*P)
	Lx, Ly := elliptic.Unmarshal(curve, l)
	if Lx == nil {
		return nil, nil, errors.New("failed to unmarshal L")
	}
	// Unmarshal M generator
	Mx, My := elliptic.Unmarshal(curve, spake2pM)
	if Mx == nil {
		return nil, nil, errors.New("failed to unmarshal SPAKE2+ M generator")
	}
	// Compute w0*M
	w0Mx, w0My := curve.ScalarMult(Mx, My, w0)
	// Negate w0*M: (x, y) -> (x, p-y)
	p := curve.Params().P
